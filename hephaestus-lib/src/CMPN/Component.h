...
namespace CMPN {
    ...
    public:
        Component(const ComponentID& cid, const ObjectID& oid) : Implements<Interface>(cid, oid) {
            (void)&m_CTID;
        }
        virtual ~Component() {
        }
        
        static ComponentBase* create(const ComponentID& cid, const ObjectID& oid) { return new Implementation(cid, oid); }
