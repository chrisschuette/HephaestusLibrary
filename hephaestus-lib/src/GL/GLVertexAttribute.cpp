...
#define NAME "GLVertexAttribute"

namespace GL {

    GLVertexAttribute::GLVertexAttribute(std::string desc) {
    ...
    void GLVertexAttribute::enable() {
        if (m_index == -1) {
            ...
            if (program == 0) {
                L(warning) << "Could not enable vertex attribute " + m_name + ": no program.";
                return;
            }
            m_index = glGetAttribLocation(program, m_name.c_str());
            if (m_index == -1) {
                L(warning) << "Could not enable vertex attribute: " + m_name + ": no attribute.";
                return;
            }
        }
